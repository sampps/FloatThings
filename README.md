...
### 拖拽排序
在待办列表中，长按卡片右侧拖拽手柄可自由调整顺序。拖动时卡片以幽灵（Ghost）形态跟随指针移动，其他卡片实时弹开为落点留出空位；松手后幽灵卡片平滑飞入目标位置并与真实卡片无缝交接，全程无闪烁。

- 基于 Pointer Events 实现，兼容 Tauri WebView（不依赖 HTML5 Drag API）
- 拖拽期间 DOM 顺序不变，仅用 `translateY` 做视觉位移，松手时一次性提交新顺序
- 槽位让开动画 0.8s，落位动画 0.5s，均采用 `cubic-bezier(0.22, 1, 0.36, 1)` 先快后慢曲线
...
- [sampps](https://github.com/sampps) — 原作者
- [Ouyangheyi](https://github.com/Ouyangheyi) — 拖拽排序功能
